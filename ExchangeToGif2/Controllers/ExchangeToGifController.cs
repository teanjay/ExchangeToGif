...
namespace ExchangeToGif.Controllers
{
    ...
    public class ExchangeToGifController : ControllerBase
    {
        ...
        public ExchangeToGifController(HttpClient httpClient)
        {
        ...
        {
                var todayExchangeRate = await _httpClient.GetAsync("https://openexchangerates.org/api/latest.json?app_id=cbeb6a28a2474099965acd81d3d98a29&symbols=RUB");
                var yesterdayExchangeRate = await _httpClient.GetAsync("https://openexchangerates.org/api/historical/" + DateTime.Now.Date.AddDays(-1).ToString("yyyy-MM-dd") + ".json?app_id=cbeb6a28a2474099965acd81d3d98a29&symbols=RUB");
                ...
                var gif = await getGif(resultTER.rates["RUB"] > resultYER.rates["RUB"]);
                    return new MsgResult(gif);
                ...
                async Task<byte[]> getGif(bool exchangeRate)
                {
                    ...
                    if (exchangeRate)
                    {
                        gifResponse = await _httpClient.GetAsync("https://api.giphy.com/v1/gifs/random?api_key=ajm4Eh27WEgd0uvKNyYJjYBfGloyDL5z&tag=rich");
                    }
                    else
                    {
                        gifResponse = await _httpClient.GetAsync("https://api.giphy.com/v1/gifs/random?api_key=ajm4Eh27WEgd0uvKNyYJjYBfGloyDL5z&tag=broke");
                    }
                    ...
                    var gifUrl = await _httpClient.GetAsync(gifResponseJson.data.images.downsized.url);
